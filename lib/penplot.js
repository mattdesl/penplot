...
export default function penplot (createPlot, opt = {}) {
  ...
  function setup () {
    ...
    if (plot.sequence) {
      loop = createLoop(renderSequenceFrame).start();
    } else if (plot.animate) {
      loop = createLoop(draw).start();
    } else {
      draw();
    }
  }
  ...
  function renderSequenceFrame () {
    const duration = defined(plot.duration, 1);
    const fps = defined(plot.fps, 25);
    const interval = 1 / fps;
    const totalFrames = Math.ceil(fps * duration);
    if (currentFrame >= totalFrames) return;
    // Now draw real PNG output
    if (plot.saveSequence !== false) {
      // this will call draw after rendering the frame
      save({
        frame: currentFrame,
        totalFrames
      });
    } else {
      // draw the frame without saving
      draw(interval, currentFrame, totalFrames);
    }
    currentFrame++;
    if (currentFrame >= totalFrames) {
      if (loop) loop.stop();
      console.log(`‣ Sequence complete: ${totalFrames} saved.`);
    }
  }

  function draw (dt = 0, frame = 0, totalFrames = 1) {
    context.save();
    context.scale(
    ...
    plot.draw(dt, frame, totalFrames);
    context.restore();
  }
  ...
  function save (opt = {}) {
    ...
    const currentFrame = defined(opt.frame, 0);
    const totalFrames = defined(opt.totalFrames, 1);
    canvas.width = width;
    canvas.height = height;
    canvasWidth = canvas.width;
    canvasHeight = canvas.height;
    pixelRatio = 1;
    draw(0, currentFrame, totalFrames);
